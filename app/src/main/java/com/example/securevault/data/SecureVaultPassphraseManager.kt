...
object SecureVaultPassphraseManager {
  ...
  private const val PREFS_FILE = "securevault_encrypted_prefs"
  private const val KEY_DB_PASSPHRASE = "securevault_db_encryption_key_v1"
  ...
  fun getOrCreatePassphrase(context: Context): ByteArray {
    ...
    // Generate dedicated random 256-bit (32-byte) key for SecureVault database
  ...
  private fun getEncryptedPrefs(context: Context): SharedPreferences {
    return try {
      ...
      Log.w(TAG, "EncryptedSharedPreferences unavailable for SecureVault: ${e.message}. Using private SharedPreferences fallback.")
      isFallbackMode = true
